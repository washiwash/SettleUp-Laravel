...
class _WalletPageState extends State<WalletPage> {
  ...
  Future<void> _fetchUserDetails() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          // Save user details to the state
  ...
  Future<void> _saveTransactions() async {
    ...
    await prefs.setString(
        'archivedItems', jsonEncode(archivedItems)); // Save archived items
  }
  ...
  //   //   setState(() {
  //   //     receiveList =
  //   //         List<Map<String, dynamic>>.from(jsonDecode(receiveListString));
  //   //   });
  ...
  void _archiveItem(int index, bool isReceive) {
    setState(() {
      final item =
          isReceive ? receiveList.removeAt(index) : payList.removeAt(index);
  ...
  void addTransaction(Map<String, dynamic> transaction, bool isReceive) {
    ...
    if (!targetList.contains(transaction)) {
      ...
      print(
          'Added transaction to ${isReceive ? "receiveList" : "payList"}: $targetList');
    } else {
  ...
  double getResponsiveFontSize(BuildContext context, double baseFontSize) {
    ...
    return baseFontSize *
        (screenWidth / 375); // 375 is the base width for scaling
  }

  Future<void> _storeDebtor(Map<String, dynamic> debtorData) async {
    final url = Uri.parse('http://127.0.0.1:8000/api/debtors');
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString('auth_token') ?? '';
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        // Update the relevant list (receive or pay)
        if (debtorData['debt_type'] == 'receivable') {
          setState(() {
            receiveList.add({
              'name': debtorData['name'],
              'amount': debtorData['amount_to_borrow'],
              'startDate': debtorData['start_date'],
              'dueDate': debtorData['due_date'],
              'interestRate': debtorData['interest_rate'],
              'totalAmount': debtorData['total_amount'],
            });
          });
        } else {
          setState(() {
        ...
        _saveTransactions(); // Save the updated lists
      } else {
        final responseData = json.decode(response.body);
        ScaffoldMessenger.of(context).showSnackBar(
                      ...
                      child: Row(
                        children: [
                          _buildTabButton(
                            ...
                            onTap: () =>
                                setState(() => isReceiveSelected = true),
                          ),
                          _buildTabButton(
                            text: 'Pay',
                            isSelected: !isReceiveSelected,
                            onTap: () =>
                                setState(() => isReceiveSelected = false),
                          ),
                        ],
                      ),
                    ),
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            borderRadius: BorderRadius.only(
              topLeft:
                  text == 'Receive' ? const Radius.circular(20) : Radius.zero,
              bottomLeft:
                  text == 'Receive' ? const Radius.circular(20) : Radius.zero,
              topRight: text == 'Pay' ? const Radius.circular(20) : Radius.zero,
              bottomRight:
                  text == 'Pay' ? const Radius.circular(20) : Radius.zero,
            ),
            color: isSelected
                ? (text == 'Receive'
                  ...
                  child: ListTile(
                    ...
                    subtitle: Column(
                      ...
                      children: [
                        ...
                        Text(
                            'Duration: ${item['startDate']} - ${item['dueDate']}'),
                        ...
                        Row(
                          ...
                          children: [
                            IconButton(
                              ...
                              onPressed: () => showDialogInterestTransaction(
                                  context, index, isReceive),
  ...
  void showDialogInterestTransaction(
      BuildContext context, int index, bool isReceive) {
    ...
    final double interestRate =
        double.tryParse(transaction['interestRate']?.toString() ?? '0') ?? 0.0;

    // Calculate the predicted interest
    final DateTime startDate =
        DateFormat('MMM dd, yyyy').parse(transaction['startDate']);
    final DateTime dueDate =
        DateFormat('MMM dd, yyyy').parse(transaction['dueDate']);
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            _buildDetailRow('Transaction Duration',
...
// Helper method to create consistent detail rows
  ...
  void _addInterestToTransaction({
    ...
    final double totalAmount = amount * (interestRate / 100);

    setState(() {
      final double amount = double.parse(transaction['amount']);
      final double interest = amount * (interestRate / 100);
      transaction['calculatedInterest'] = interest;
    });
                    ...
                    actions: [
                      TextButton(
                        ...
                        style: TextButton.styleFrom(
                          backgroundColor:
                              const Color(0xFF432818), // Cool background color
                          shape: RoundedRectangleBorder(
    ...
    return FloatingActionButton(
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(
            25), // Adjust this value for a more oblong shape
      ),
              ...
              ElevatedButton(
                onPressed: () async {
                  if (formKey.currentState?.validate() ?? false) {
                    final prefs = await SharedPreferences.getInstance();
                    final userId =
                        prefs.getString('user_id') ?? ''; // Retrieve user ID
                    ...
                    final double totalAmount =
                        amountToBorrow + (amountToBorrow * interestRate / 100);

                    final transaction = {
                      'usr_id': userId, // Include user ID in the payload
                      ...
                      'due_date': DateFormat('yyyy-MM-dd').format(
                          DateFormat('MMM dd, yyyy')
                              .parse(dueDateController.text.trim())),
                      'interest_rate': interestRate,
                      'debt_type': isReceiveSelected ? 'receivable' : 'payable',
                      'total_amount': totalAmount,
                    };

                    Navigator.pop(context); // Close the dialog
                    _storeDebtor(transaction); // Send data to the backend
                  }
                },
